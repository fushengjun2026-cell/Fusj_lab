...
function reset(){board=initialBoard.map(row=>row.slice());turn="w";selected=null;selectedMoves=[];lastMove=null;gameOver=false;render();updateStatus()}
boardElement?.addEventListener("click",handleSquare);resetButton?.addEventListener("click",reset);reset();
